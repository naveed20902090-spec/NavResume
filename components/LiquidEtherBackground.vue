<template>
  <div ref="mountRef" :class="['liquid-ether-container', props.className]" :style="props.style" />
</template>

<script setup lang="ts">
import { nextTick } from 'vue'
import * as THREE from 'three'
import './LiquidEtherBackground.css'
...
`
  ...
  class ExternalForce extends ShaderPass {
    ...
    override update(forceProps: any) {
      const forceX = (Mouse.diff.x / 2) * forceProps.mouse_force
      const forceY = (Mouse.diff.y / 2) * forceProps.mouse_force
      const cursorSizeX = forceProps.cursor_size * forceProps.cellScale.x
      const cursorSizeY = forceProps.cursor_size * forceProps.cellScale.y
      const centerX = Math.min(
        Math.max(Mouse.coords.x, -1 + cursorSizeX + forceProps.cellScale.x * 2),
        1 - cursorSizeX - forceProps.cellScale.x * 2
      )
      const centerY = Math.min(
        Math.max(Mouse.coords.y, -1 + cursorSizeY + forceProps.cellScale.y * 2),
        1 - cursorSizeY - forceProps.cellScale.y * 2
      )
      const uniforms = (this.mouse.material as THREE.RawShaderMaterial).uniforms
      uniforms.force.value.set(forceX, forceY)
      uniforms.center.value.set(centerX, centerY)
      uniforms.scale.value.set(forceProps.cursor_size, forceProps.cursor_size)
      super.update()
    }
  }
