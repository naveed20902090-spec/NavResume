<template>
  <div ref="mountRef" class="liquid-ether-container" aria-hidden="true" />
</template>
...
`
  ...
  class Simulation {
    ...
    getFloatType() {
      ...
      if (!renderer) return THREE.HalfFloatType
    ...
    createShaderPass() {
      ...
      this.pressurePass = new Pressure({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, src_p: this.fbos.pressure_0, src_v: this.fbos.vel_viscous0, dst: this.fbos.vel_0, dt: this.options.dt })
    }
  ...
  class Output {
    ...
    constructor() {
      this.simulation = new Simulation({
        mouse_force: settings.mouseForce,
        cursor_size: settings.cursorSize,
        isViscous: settings.isViscous,
        viscous: settings.viscous,
        iterations_viscous: settings.iterationsViscous,
        iterations_poisson: settings.iterationsPoisson,
        dt: settings.dt,
        BFECC: settings.BFECC,
        resolution: settings.resolution,
        isBounce: settings.isBounce
      })
      this.scene = new THREE.Scene()
      this.camera = new THREE.Camera()
  ...
  const webgl = new WebGLManager({
    $wrapper: container,
    autoDemo: settings.autoDemo,
    autoSpeed: settings.autoSpeed,
    autoIntensity: settings.autoIntensity,
    takeoverDuration: settings.takeoverDuration,
    autoResumeDelay: settings.autoResumeDelay,
    autoRampDuration: settings.autoRampDuration
  })
